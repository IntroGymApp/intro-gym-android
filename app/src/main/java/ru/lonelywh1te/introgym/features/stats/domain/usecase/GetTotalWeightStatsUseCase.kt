...
class GetTotalWeightStatsUseCase(
    ...
    operator fun invoke(period: StatsPeriod): Flow<Result<Map<LocalDate, Float>>> {
        return repository.getTotalWeightStats(period).map { result ->
            var resultData: Result<Map<LocalDate, Float>>? = null
    ...
    private fun groupAndCalculateTotalWeight(list: List<Pair<LocalDate, Float>>, period: StatsPeriod): Map<LocalDate, Float> {
        ...
        return if (period is StatsPeriod.Year) {
            allDates
